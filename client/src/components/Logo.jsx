...
export default function Logo({ size='text-2xl', stacked=false }){
  const mainSrc = '/assets/Outside App Logo.jpg'
  const altSrc = '/assets/Main Logo.jpg'
  ...
  return (
    <div className={`flex items-center ${stacked?'flex-col gap-1':'gap-3'}`}>
      ...
      hasAlt ? <img src={altSrc} alt="DWARLY" className="h-10 w-10 rounded-2xl shadow-glow"/> :
