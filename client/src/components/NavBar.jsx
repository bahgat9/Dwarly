    ...
    <div className="sticky top-0 z-40 backdrop-blur bg-brand-800/80 border-b border-white/10">
      <div className="mx-auto max-w-7xl px-3 md:px-4 py-2 md:py-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3 text-white">
          ...
          <NavLink to="/matches" label="Find Jobs / وظائف التدريب" />
          {session ? (
            <button onClick={onLogout} className="inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-white/10 hover:bg-white/20 transition shadow"><LogOut size={16}/>Logout</button>
          ) : (
      ...
      {open && (
        <div className="md:hidden px-3 pb-4 text-white space-y-2">
          ...
          {session ? (
            <button onClick={()=>{onLogout(); setOpen(false);}} className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-2xl bg-white/10 hover:bg-white/20 transition shadow"><LogOut size={16}/>Logout</button>
          ) : (
            <MobileLink to="/login" label={<span className="inline-flex items-center gap-2"><LogIn size={16}/>Login</span>} onClick={()=>setOpen(false)} />
          )}
        </div>
      )}
    </div>
